...
func uploadBuildableTestSource(t *testing.T, manager *Manager, pluginID string) ArtifactRecord {
	...
	manifest := Manifest{
		SchemaVersion: SchemaVersion, ID: pluginID, Name: "Buildable Source", Version: "0.1.0",
		ArtifactType: ArtifactTypeSource, Runtime: RuntimeManifest{Type: RuntimeGoPlugin, EntrySymbol: "Plugin"},
		Build:      BuildManifest{Type: BuildTypeGo, Entry: ".", GoVersion: runtime.Version(), Output: RuntimeEntry},
		APIVersion: APIVersion, SDKModule: "github.com/tursom/mc-gateway/plugin/api", SDKModuleVersion: "v0.1.0",
		GoVersion: runtime.Version(), GOOS: runtime.GOOS, GOARCH: runtime.GOARCH,
		ExtensionPoints: []ExtensionPoint{{Type: "hook", Key: ExtensionUpstreamConnect}},
...
func (p *pluginImpl) Init(gateway api.Gateway) error {
	return api.RegisterUpstreamConnectHandlerV2(gateway, func(api.UpstreamConnectRequestV2) error { return nil })
}
`
	packagePath := writeTestMCGP(t, map[string][]byte{
		"manifest.json": manifestBytes,
	...
	source, err := manager.UploadSource(context.Background(), ArtifactUpload{SourcePath: packagePath, FileName: pluginID + "-source.mcgp", Actor: "admin"})
	if err != nil {
		t.Fatal(err)
	}
	return source
...
func (a *reconcileLifecycleAdapter) Start(_ context.Context, prepared RuntimePrepared, artifact ArtifactRecord, _ PluginRecord, gateway *Gateway) (RuntimeInstance, error) {
	...
	if !a.skipHandlers[artifact.ID] {
		handler := a.handlers[prepared.DesiredGeneration]
		if handler == nil {
			handler = func(api.UpstreamConnectRequestV2) error { return nil }
		}
		if err := api.RegisterUpstreamConnectHandlerV2(gateway, handler); err != nil {
			return RuntimeInstance{}, err
		}
	}
...
func (a *fakeAdapter) Load(_ context.Context, artifact ArtifactRecord, _ PluginRecord, gateway *Gateway) (api.Plugin, error) {
	...
	if !a.initOnly {
		if err := api.RegisterUpstreamConnectHandlerV2(gateway, func(api.UpstreamConnectRequestV2) error { return nil }); err != nil {
			return nil, err
		}
	}
	if a.initHook != nil {
