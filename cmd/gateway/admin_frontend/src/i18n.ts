...
const localizedMessages: Partial<Record<Language, Record<string, string>>> = {
  zh: {
    ...
    "future runtime gates sandbox_process and wasm must be enabled before activation": "启用前必须先打开 sandbox_process 和 wasm 未来运行时开关",
    ...
    "user updated": "用户已更新",
    "wasm runtime requires sandbox-process service mode": "wasm 运行时需要 sandbox-process 服务模式",
    "wasm runtime is reserved; schema and validation checks do not provide a WASM data-plane": "wasm 运行时已预留；schema 和校验检查不提供 WASM 数据面",
    "wazero runtime is implemented for low-risk validation extension points but disabled unless future runtime gates enable wasm": "wazero 运行时已为低风险校验扩展点实现，但在未来运行时开关启用 wasm 前保持禁用",
