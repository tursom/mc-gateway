...
const localizedMessages: Partial<Record<Language, Record<string, string>>> = {
  zh: {
    ...
    "go-plugin-process supports upstream.connect/v2 takeover with persisted crash policy and per-node crash isolation; fd-live migration, sandbox enforcement, full isolation, and non-Linux process-table orphan discovery are not implemented": "go-plugin-process 支持 upstream.connect/v2 连接接管，包含持久化崩溃策略与单节点崩溃隔离；暂未实现 fd 热迁移、沙箱强制、完整隔离和非 Linux 进程表孤儿发现",
    ...
    "sandbox-process data-plane is partial; selected request/response extension points and upstream.connect/v2 takeover use the sandbox stream relay with drain-only lifecycle; broader isolation and governance hardening remain incomplete": "sandbox-process 数据面部分可用；已支持选定请求/响应扩展点，upstream.connect/v2 连接接管通过 sandbox stream relay 和 drain-only 生命周期承载，完整隔离和治理仍需继续加固",
    ...
    "user updated": "用户已更新",
    "wasm runtime only supports low-risk extension points; upstream.connect/v2, network, file, and other high-risk extension points are not supported": "wasm 运行时仅支持低风险扩展点；不支持 upstream.connect/v2、网络、文件和其他高风险扩展点",
...
const uiTranslations: Partial<Record<Language, Record<string, string>>> = {
  zh: {
    "Active": "当前",
    "Active mode": "生效模式",
    "Active sessions": "活跃连接会话",
    "Active connection sessions": "活跃连接会话",
    "Active support": "生效支持",
    "Adapter": "适配器",
    "Advisory ID": "公告 ID",
    "Artifact": "制品",
    ...
    "No active connection sessions": "暂无活跃连接会话",
    "No artifacts": "暂无制品",
    "No builds": "暂无构建",
    ...
    "Runtime type": "运行时类型",
    "Sandbox data plane is blocked by the runtime gate or environment self-check.": "Sandbox 数据面被运行时开关或环境自检阻断。",
    "Sandbox environment": "Sandbox 环境",
